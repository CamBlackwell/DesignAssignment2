import App from "../App";
import TimeButton from "./TimeButtonForReal";
function Dashboard({PlantsData,dashTime}) {

  let index = 0;

  let exportTime = dashTime;
  let test = PlantsData;

  let waterCount = 0;
  let overdueCount = 0;
  let fineCount = 0;

  //let internalfineCount = 0;
  //let internalwaterCount = 0;

  let refresh = true;

  //let test = App();

 // let dashTime = "aa" //test.getAppTime();

  //dashTime = App.time;

  console.warn(dashTime);

    while (index < PlantsData.length) {
      if (dashTime  > PlantsData[index].urgency) {
        overdueCount = overdueCount + 1
      }
      else if (dashTime == PlantsData[index].urgency) {
        waterCount = waterCount + 1;
      }
      else if (dashTime < PlantsData[index].urgency) {
        fineCount = fineCount + 1;
      }
      index = index + 1;
    }
  

  // console.warn(dashTime); <button onClick={() => Dashboard()}>Load DAsh</button>
  return (
    <div className="dashboard-container">
        <h2>Dashboard</h2>
        <h3>Total Number of Plants: {PlantsData.length} Plants due: {waterCount} Plants Overdue: {overdueCount} Plants OK: {fineCount}</h3>
        
    </div>
  );
}

export default Dashboard;